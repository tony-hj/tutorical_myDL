...
class Generator(object):
    ...
    def get_random_data_mosaic(self,annotation_line, input_shape, random=True, hue=.1, sat=1.5, val=1.5, proc_img=True):
        '''random preprocessing for real-time data augmentation'''
        h, w = input_shape
        min_offset_x = 0.4
        min_offset_y = 0.4
    ...
    def get_random_prue_data(self, annotation_line, input_shape, random=True, jitter=.1, hue=.1, sat=1.3, val=1.3, proc_img=True):
        '''r实时数据增强的随机预处理'''
        line = annotation_line.split()
        image = Image.open(line[0])
        iw, ih = image.size
    ...
    def generate(self, train=True):
        while True:
            ...
            if -1*(len(self.train_lines)%4) == 0:
              lines = self.train_lines
            else:
              lines = self.train_lines[:-1*(len(self.train_lines)%4)]# 舍弃多余数据，使得总大小为4的倍数
            inputs = []
            targets = []
            for start in range(0,len(lines),4):  
                annotation_line = lines[start:start+4]
                # print(annotation_line)
                img,y=self.get_random_data_mosaic(annotation_line,self.image_size[0:2]) # 替换试试？
                y = np.array(y)
                if len(y)==0:
                    continue
