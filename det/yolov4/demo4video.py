...
def detect(cfgfile, weightfile, root):
    m = Darknet(cfgfile)
    ...
    for each in os.listdir(root):
      imgfile = os.path.join(root,each)
      ...
      plot_boxes(img, boxes, os.path.join('frames_2',each), class_names)
...
if __name__ == '__main__':
    if len(sys.argv) == 4:
        ...
        root = sys.argv[3]
        detect(cfgfile, weightfile,root)
